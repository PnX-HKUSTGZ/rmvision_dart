...
`active` 模式下，`target_id=1` 写 base 图像，`target_id=0` 写 outpost 图像；`/livox/accum_points` 会按 `ROSBAG_CLOUD_HZ` 低频保留，`Send_fused`、`/Send`、状态和 `/rosout` 仍会保留。
...
如需控制赛后点云投影视频的绘制点数：
```bash
EXTRACT_CLOUD_MAX_POINTS=30000 ./RECORD/extract_all_bags.sh
```
